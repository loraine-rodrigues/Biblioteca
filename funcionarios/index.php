...
require '../conexao.php';
try {
    $query = $conexao->query("CALL buscarFuncionarios()");
}
catch (mysqli_sql_exception $exception) {
    echo "Erro ao buscar funcionários : (" . $conexao->errno . ") " . $conexao->error;
}
$conexao->close();
?>
<!doctype html>
...
<table class="table">
    ...
    <?php while ($resultado = $query->fetch_array(MYSQLI_ASSOC)): ?>
        <tr>
            ...
            <td><?php echo $resultado['nm_funcionario'] ?></td>
            <td><?php echo $resultado['nm_endereco'] ?></td>
            <td><?php echo $resultado['cd_telefone'] ?></td>
            <td><?php echo $resultado['vl_salario'] ?></td>
            <td><?php echo $resultado['nm_funcao'] ?></td>
